...
int Refractor(int argc, char* argv[])
{

    system("# Diehhria Console");

    system("color 0A");
    if (argc < 3)
    {
        system("title recker error!");

        std::cout << "ins parameters: host port [nick] [user]" << std::endl;
        return 1;
    }
    ...
    {
        std::cout << "socked and connecting..." << std::endl;
